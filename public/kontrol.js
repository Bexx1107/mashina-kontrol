 ...
 * 
 * Author: Žan Zupančič / Mashina Studio (https://mashina-studio.eu)
 */
...
• You can edit this speech script anytime by tapping "Edit Notes". Notes are saved automatically on this device.`
  ...
  // ─── Gamepad Layout Drag & Scale Engine ──────────────────────────────
  ...
  function getModuleTransform(moduleId) {
    const layoutMap = state.layoutTransforms[state.currentLayout] || {};
    return layoutMap[moduleId] || { x: 0, y: 0, scale: 1.0 };
  }

  function setModuleTransform(moduleId, transform) {
    if (!state.layoutTransforms[state.currentLayout]) {
      state.layoutTransforms[state.currentLayout] = {};
    }
    state.layoutTransforms[state.currentLayout][moduleId] = transform;
  }
  ...
  function applyAllLayoutTransforms() {
    const layoutMap = state.layoutTransforms[state.currentLayout] || {};
    const modules = document.querySelectorAll('.dpad-cross, .face-diamond, .joystick-pod, .ps-center-touchpad-zone, .bumper-group');
    modules.forEach(mod => {
  ...
  function adjustSelectedScale(delta) {
    if (!state.selectedModuleId) {
      const firstMod = document.querySelector('.customizer-draggable');
      if (firstMod) {
  ...
  function resetLayoutPositions() {
    delete state.layoutTransforms[state.currentLayout];
    localStorage.setItem('mashina_layout_transforms', JSON.stringify(state.layoutTransforms));
  ...
  function setupUIControls() {
    ...
    // Settings Modal (Triggered by Wrench Icon in Header)
    if (el.editModeBtn && el.settingsModal) {
